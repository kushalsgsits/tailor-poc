...
public interface Cache<K, V> {
	 ...
	 * specified.
	 * 
	 * @param key           key of the entry that is used as handle, to retrieve at
	 *                      a later point of time or to delete the entry before its
	 *                      expiry time.
	 * @param value         The actual object that is to be cached.
	 * @param secondsToLive expiry time in seconds. so if the cache entry needs to
	 *                      be expired in 5 minutes, expiryTime needs to be set as
	 *                      300. If the cache entry doesn't need an expiry time, set
	 *                      it as -1.
	 * @throws IllegalArgumentException Throws illegal argument exception, if the
	 ...
	 * @return Returns the cached entry. Returns null, if there is no entry with the
	 *         key K or if the entry has expired.
	 */
	V get(K key);
	...
	/**
	 * Removes an entry from the cache using the key K. Returns true, if the remove
	 * operation is successful. Else, returns false.
	 * 
	...
	/**
	 * Removes an entry from the cache using the key K. Returns object that has been
	 * removed. If the operation is not successful, returns <code>null</code>.
	 * 
	 * @param key The handle of the entry that has to be removed.
	 * @return Returns object that has been removed. If the operation is not
